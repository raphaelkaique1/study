...
Um *`component`* **é um trecho de código que contém a lógica e as instruções que determinam os dados a serem retornados para o HTML**, sendo **obrigatório** *que a 1ª letra no nome de funções componentes seja maiúscula*. Este código contem um código JSX será utilizada na construção da interface usando a sintaxe de XML para a escrita do documento, e nessa *tag* criada **cada atributo é uma chave para acessar os valores dentro das _props_**. Em outras palavras, o componente é a construção do elemento que será utilizado na função *"main"* que é quem irá gerenciar e renderizar cada componente da aplicação de acordo com as regras e lógica definida.<br/>
**Cada componente pode ter um estado**, *este que é uma variável que armazena as informações que mudam dentro do componente*, como os dados de um formulário ou a cor de um botão. Quando o usuário interage com a aplicação realizando ações como cliques em botões ou preenchimento de formulários, o estado dos componentes é atualizado e essas mudanças são refletidas na UI. Isso é feito através do uso de callbacks — que são funções chamadas quando uma ação é executada por um usuário.<br/>
...
**Hooks**<br/>
React Hooks são funções que permitem usar utilitários dos componentes de classe em componentes funcionais, como gerenciar estado além de outros recursos do React em componentes de funções sem a necessidade da criação de classes. Ou seja, Hooks são métodos especiais que adicionam recursos aos componentes funcionais, em outras palavras, são métodos prontos utilizados para realizar algum tipo de modificação no componente do documento. Só é possível usar um Hook — customizado ou não — dentro de um componente funcional. Alguns exemplos dos Hooks mais utilizados são o `useState` — que adiciona um estado local ao componente, `useEffect` — que executa o efeito colateral da alteração de estado do componente, também o `useContext` — que gerencia o contexto global, entre outros. Um componente pode usar Hooks mas não precisa deles para funcionar, enquanto um Hook apenas funciona se usado em um componente. Vejamos alguns exemplos:
```jsx
...
- `timesClicked`    → valor atual (inicialmente 0 definido em useState(0))
- `setTimesClicked` → função que altera o valor
- `useState(0)`     → define o valor inicial com o parâmetro recebido, neste caso `0`

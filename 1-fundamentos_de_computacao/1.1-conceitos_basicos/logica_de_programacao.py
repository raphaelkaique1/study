...
-PENSAMENTO COMPUTACIONAL
 ...
 Refere-se ao processo de pensamento envolvido na expressão de soluções em passos computacionais ou algoritmos que podem ser 
implementados no computador. De forma geral, é uma formulação e resolução de problemas computacionais (sistemático e eficiente).
...
dividimos a escrita em partes menores que vão sendo construídas aos poucos.
 ...
 É preciso compreender como executar cada etapa de um pensamento computacional. "Quebrar" o problema é o primeiro passo da 
...
>PADRÕES
 ...
 Ação de descrever o que vai acontecer com base em evento anteriores. Na computação, é comum utilizar estruturas de repetição, 
por exemplo, para blocos de códigos semelhantes que se repetem de alguma forma.
...
Classe: É uma subdivisão mais específica de uma categoria. No exemplo anterior, a categoria "aves" pode ser subdivida em: "aquáticas", 
...
> let:
 - Introduz o escopo de bloco, o que significa que a variável é visível apenas dentro do bloco onde foi declarada (por exemplo,
dentro de um `if`, `for` ou {}).
...
> define*:
 ...
 - Processamento Prévio: O #define é uma diretiva de pré-processador. Isso significa que ele é processado pelo compilador
antes de qualquer compilação real do código.
 - Sem Verificação de Sintaxe: Quando você usa #define, o pré-processador simplesmente substitui cada ocorrência do token
definido pelo texto correspondente, sem verificar a sintaxe. Portanto, é importante garantir que a substituição seja feita
corretamente para evitar erros de compilação difíceis de depurar.
...
associado à macro definida por #define, é apenas uma substituição de texto.
 - Substituição Direta de Texto: O #define não é afetado por escopos de bloco ou por escopo de função. Isso significa que a
substituição ocorrerá em todo o código, independentemente do escopo.
...
de código, ele não é recomendado para substituição de tipos complexos, estruturas de controle ou funções. Para isso, normalmente
são usados tipos de dados constantes ou funções inline.
 - Escopo Global: As macros definidas por #define têm um escopo global, o que significa que elas são válidas em todo o código
após a definição, até o final do arquivo ou até que sejam substituídas por outra diretiva #undef.
...
>>>FUNÇÕES
 ...
 "Subalgoritmo", "bloco", "método", "função", "subprograma", "subrotina"... São instruções que realizam tarefas específicas, são
trechos de códigos com instruções/objetivos específicos que podem ser chamadas dentro do código principal. Ajudam na decomposição
e modularização do algoritmo para torná-lo mais legível.
